...
+ (WToast *)__createWithText:(NSString *)text {
	...
	tmpRect.size.width = MIN(sz.width + 20, screenWidth - x * 2.0f);
...
//    toast.backgroundColor = [UIColor colorWithHex:0xf12e7f andAlpha:0.8f];
...
+ (void)showWithText:(NSString *)text {
	[WToast showWithText:text length:kWTLongShort];
}
...
//	UIWindow *mainWindow = [[UIApplication sharedApplication] keyWindow];
    UIWindow *mainWindow = [[UIApplication sharedApplication].windows objectAtIndex:([UIApplication sharedApplication].windows.count - 1)];
